...
// Pure PDF generation, no gating logic (e.g. "must be Delivered") — that's
// a decision for whoever calls this (the download route restricts it,
// email sending doesn't, since a customer should get an invoice at order
// confirmation time, long before delivery).
export function generateInvoicePdf(order: IOrder): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Was missing entirely — an order with a promo code applied showed a
    // total on the invoice that didn't reconcile with subtotal + delivery.
    if (order.discount > 0) {
      doc.text(`Discount${order.promoCode ? ` (${order.promoCode})` : ""}`, col.price, y);
      doc.text(`- ${formatINR(order.discount)}`, col.total, y);
      y += 18;
    }
    ...
    doc.text(`Payment: ${order.paymentMethod.toUpperCase()} - ${order.paymentStatus}`, 50, y);
